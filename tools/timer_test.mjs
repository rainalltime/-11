...
const edge = spawn(edgeExe, ['--headless=new','--disable-gpu','--no-sandbox','--remote-debugging-port='+PORT,'--user-data-dir=/tmp/cdp-timer','about:blank'], { stdio: 'ignore' });
await sleep(2500);
let targets;
...
console.log('第1关(教学3猪)时间:', await evl(`document.getElementById('timer').textContent`));
// 选第 50 关(约149猪)
...
console.log('第50关状态:', await evl(`document.getElementById('status').textContent`));
console.log('第50关时间:', await evl(`document.getElementById('timer').textContent`));
