...
const edge = spawn(edgeExe, ['--headless=new','--disable-gpu','--no-sandbox','--remote-debugging-port='+PORT,'--user-data-dir=/tmp/cdp-tbar','about:blank'], { stdio: 'ignore' });
await sleep(2500);
let targets;
...
console.log('教学关(3猪):', await evl(`document.getElementById('timer').textContent`), '| 刻度数:', await evl(`document.querySelectorAll('#time-bar .tick').length`));
console.log('教学关刻度位置(30/20/10s):', await evl(`[...document.querySelectorAll('#time-bar .tick')].map(t=>t.style.left).join(',')`));
// 选第 50 关
await evl(`document.getElementById('btn-menu').click()`); await sleep(400);
await evl(`[...document.querySelectorAll('#menu-grid button')].find(b=>b.textContent.trim()==='50').click()`); await sleep(800);
console.log('第50关:', await evl(`document.getElementById('status').textContent`));
console.log('第50关时间:', await evl(`document.getElementById('timer').textContent`), '| 刻度:', await evl(`document.querySelectorAll('#time-bar .tick').length`));
console.log('填充宽度%:', await evl(`document.getElementById('time-fill').style.width`));
